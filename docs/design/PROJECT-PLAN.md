...
│  Phase 1: 核心引擎增强          ████████████████░░░░  80% 🔴 进行中         │
│  Phase 2: IDE 交互体验          ██████████████░░░░░░  70% 🔴 进行中         │
│  Phase 3: 高级可视化            ░░░░░░░░░░░░░░░░░░░░   0% ⚪ 待开始         │
│  Phase 4: AI 集成               ░░░░░░░░░░░░░░░░░░░░   0% ⚪ 待开始         │
│  Phase 5: 质量保证与发布        ░░░░░░░░░░░░░░░░░░░░   0% ⚪ 待开始         │
│                                                                              │
│  ════════════════════════════════════════════════════════════════════════   │
│  当前重点：完成 Phase 1-2 核心功能，为 v1.0 首发做准备                      │
│  ════════════════════════════════════════════════════════════════════════   │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
| 版本 | 包含内容 | 目标时间 | 状态 |
|------|---------|---------|------|
| **v0.5** | 基础 IDE + 静态执行流分析 | - | 🔴 当前开发中 |
| **v1.0** | 完整 IDE + 场景化数据流 + Linux 内核知识库 | - | ⚪ 规划中 |
| **v1.5** | + 本地 AI 辅助 | - | ⚪ 规划中 |
| **v2.0** | + Android 支持 | - | ⚪ 远期 |
...
- [x] 目录树 + 文件管理
- [x] 执行流可视化 (React Flow)
- [x] 异步机制识别 (WorkQueue/Timer/IRQ...)
- [x] 函数指针解析
- [ ] **场景化数据流分析 (核心引擎)** ← 当前重点
- [ ] Linux 内核知识库完善
- [ ] 多平台安装包 (Win/Mac/Linux)
...
4. [技术选型](#4-技术选型)
5. [详细开发计划](#5-详细开发计划)
   - [5.2 Phase 0: 原型验证](#52-phase-0-原型验证-8-周) ✅ 已完成
   - [5.3 Phase 1: 核心引擎增强](#53-phase-1-核心引擎增强-12-周) ⭐ 关键
   - [5.4 Phase 2: IDE 交互体验](#54-phase-2-ide-交互体验-12-周)
   - [5.5 Phase 3: 高级可视化](#55-phase-3-高级可视化-8-周)
   - [5.6 Phase 4: AI 集成](#56-phase-4-ai-集成-16-周) ⭐ 新增
   - [5.7 Phase 5: 质量保证与发布](#57-phase-5-质量保证与发布-12-周)
6. [UI/UX 设计](#6-uiux-设计)
7. [架构改进详细设计](#7-架构改进详细设计)
   - [7.1 增量解析架构](#71-增量解析架构)
   - [7.6 场景描述语言设计](#76-场景描述语言设计)
   - [7.7 符号执行引擎](#77-新增-crate-flowsight-symbolic) ⭐ 核心
8. [风险评估与应对](#8-风险评估与应对)
9. [资源估算](#9-资源估算)
10. [成功指标](#10-成功指标)
11. [扩展性设计](#11-扩展性设计)
...
- 🔍 **静态分析** = 不需要运行代码，纯代码阅读 ⭐ 核心
- 🧠 **语义理解** = 理解异步机制、回调模式、函数指针 ⭐ 核心
- 📊 **可视化** = 看到完整的执行流程图 ⭐ 核心
- 🤖 **AI 辅助** = 本地 AI 模型补充静态分析的不确定性 (v1.5+)
...
#### 3.2.2 动态 Trace 集成 *(可选，v2.0+)*

> ⚠️ 此功能为可选验证工具，v1.0 不包含。核心价值是**纯静态分析**。

*   **Importer**: 支持导入 `ftrace` (text/binary), `systrace`, `perf.data` 等格式。
*   **Mapper**: 将 Trace 中的函数地址/符号映射回源码位置。
*   **Visualizer**: 在静态调用图上叠加"热力图"或高亮路径，对比实际执行流。
...
**✅ 已完成** (2026-01-10):
- 通用回调模式识别完善 (队列模式、信号/槽模式)
- 场景保存/加载功能 (JSON/YAML 格式)
- 真实驱动集成测试 (USB/I2C 驱动验证)
- 结果分类系统 (Certain/Possible/Unknown 三级分类)
...
>
> **实现文件**：`crates/flowsight-analysis/src/pointer.rs`, `crates/flowsight-analysis/src/constraint.rs`
...
*   **方式5: 从Trace导入** *(可选，v2.0+)*
    > ⚠️ 此功能为可选，v1.0 不包含。核心价值是纯静态分析。
...
### 5.5 Phase 3: 高级可视化 (8 周)

**目标**: 增强可视化能力，提供专业级的代码理解视图

> ⚠️ **注意**：本阶段为**纯静态分析**，不依赖运行时 Trace！
> 
> FlowSight 的核心价值是：**不运行代码就能看懂执行流！**
...
### 5.6 Phase 4: AI 集成 (16 周) ⭐⭐⭐

**目标**: 训练并集成专用 AI 模型，弥补静态分析无法 100% 确定的部分

> **核心定位**：AI 不是猜测，而是"建议"。静态分析 + AI 预测 + 用户确认 = 100% 准确
...
│  静态分析能做到的 ────────────────→ 100% 准确显示                       │
│                                                                          │
│  静态分析无法确定的 ──→ AI 预测 ──→ 用户确认 ──→ 100% 准确             │
│                           │                                              │
│                           └── 明确标注 "🤖 AI 建议"                     │
│                                                                          │
│  特点：                                                                 │
│  • 本地部署，下载 IDE 即自带 AI 能力                                   │
│  • 完全离线使用，无需联网                                               │
│  • 专门为"看懂代码执行流"训练                                          │
│  • 用户确认后转为知识库，持续学习                                       │
│                                                                          │
...
> 📋 **完整训练数据计划见：`docs/training/TRAINING-DATA-PLAN.md`**

*   [ ] **Phase 1**: 核心驱动框架 + 异步机制 (10,000 样本)
    - 18 种总线驱动框架 (USB/PCI/Platform/I2C/SPI...)
    - 11 种异步机制 (WorkQueue/Timer/Tasklet/IRQ...)
    - 9 种同步原语 (Mutex/Spinlock/RCU...)
*   [ ] **Phase 2**: 内存/进程/文件系统 (15,000 样本)
*   [ ] **Phase 3**: 网络/块设备/安全 (15,000 样本)
*   [ ] **Phase 4**: 完整外设驱动 (20,000 样本)
*   [ ] **Phase 5**: 边界情况 + 多样化 (10,000 样本)
*   [ ] 总计：**70,000 高质量样本**，覆盖完整 Linux 内核
...
| 角色 | 模型 | 参数量 | 选择理由 |
|------|------|--------|----------|
| 教师模型 | **DeepSeek-Coder-6.7B** | 6.7B | HumanEval 73.8%，代码能力最强 |
| 学生模型 | DeepSeek-Coder-1.3B | 1.3B | 同系列，架构兼容，量化后约 1GB |
| 备选 | Qwen2.5-Coder-7B | 7B | 最新发布，如 DeepSeek 效果不佳可切换 |

**5.6.1.3 训练环境**:

> ⚠️ **本地 MacBook 不适合训练，使用云 GPU**

| 平台 | GPU | 价格 | 备注 |
|------|-----|------|------|
| **AutoDL** (推荐) | A100 40GB | ¥3-5/小时 | 国内首选，稳定 |
| Vast.ai | A100 | $1-2/小时 | 便宜 |
| RunPod | A100/H100 | $2-4/小时 | 稳定 |

**训练时间和成本预估**:

> ⚠️ **详细训练数据计划见：`docs/training/TRAINING-DATA-PLAN.md`**
>
> 包含 70,000 样本的完整规划，覆盖 Linux 内核所有子系统。
...
│  • 目标：70,000 高质量样本                                              │
│  • 覆盖：完整 Linux 内核所有子系统                                     │
│  • LLM API 辅助生成：¥1,000-2,000                                      │
│  • 核心样本人工审核：¥0（自己做）或 ¥2,000-5,000（外包）               │
│                                                                          │
│  2. 模型训练                                                            │
│  ──────────────────────────────────────                                 │
│  • 全参数微调 (A100 80GB)：30 小时 × ¥30 = ¥900                        │
│  • 知识蒸馏 (A100 80GB)：10 小时 × ¥30 = ¥300                          │
│  • 评估测试：¥100-200                                                   │
│                                                                          │
│  3. 冗余预留                                                            │
│  ──────────────────────────────────────                                 │
...
│  ════════════════════════════════════════                               │
│                                                                          │
│  总成本（推荐预算）：¥5,000-8,000                                       │
│                                                                          │
│  • 低配方案（自己标注）：¥3,000-4,000                                   │
│  • 中配方案（部分外包）：¥5,000-8,000  ← 推荐                          │
│  • 高配方案（大量人工）：¥10,000-15,000                                 │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```
...
**迭代训练策略**：
```
v1.0 (首发) → Linux 内核完整 (70,000 样本)
     ↓
v1.x (增量) → 用户反馈 + 修正 (增量训练，每次 ¥300-500)
     ↓
v2.0 (扩展) → + Android/RTOS (继续训练，¥1,500-2,000)
```
...
*   [ ] 量化：GGUF 格式，INT4 量化
*   [ ] 目标大小：约 1GB
*   [ ] 本地测试：MacBook Air M3 可流畅推理

> **详细训练步骤见：`docs/training/AI-TRAINING-GUIDE.md`**
>
> 包含完整的：
> - 云 GPU 环境配置
> - 数据收集和标注脚本
...
*   [ ] 集成 llama.cpp (Rust binding: llama-cpp-rs)
*   [ ] 支持 CPU 推理（所有用户可用）
*   [ ] 支持 GPU 加速（CUDA/Metal，可选）

**5.6.2.2 模型分发**:
*   [ ] 模型文件托管（GitHub Release / 自建 CDN）
*   [ ] 首次启动时下载模型（约 2-4GB）
*   [ ] 模型版本管理和更新

**5.6.2.3 硬件要求**:
*   最低：8GB 内存，CPU 推理（较慢但可用）
*   推荐：16GB 内存 + 独显，GPU 加速

#### 5.6.3 IDE 集成 (2 周)

**5.6.3.1 AI 预测展示**:
*   [ ] 对于"未知"的函数调用，显示 AI 预测
*   [ ] 明确标注："🤖 AI 建议：可能是 func_a"
*   [ ] 显示置信度
*   [ ] 用户可确认/否定
...
### 7.5 新增 Crate: flowsight-trace (可选，远期)

> ⚠️ **注意**：此模块为**可选功能**，v1.0 不包含！
> 
> FlowSight 的核心价值是**纯静态分析**，不依赖运行时 Trace。
> 此模块仅作为"验证工具"，用于对比静态分析结果与实际运行结果。

```
crates/flowsight-trace/          # 可选模块，v2.0+ 考虑
├── Cargo.toml
├── src/
│   ├── lib.rs
...
### 7.6 场景描述语言设计 (FlowSight Scenario Language)
...
### 7.7 新增 Crate: flowsight-symbolic ⭐ 核心
...
| **指针分析不准确** | 中 | 高 | 结合知识库 + AI 辅助预测 + 用户确认机制 |
...
| **AI 模型训练成本** | 中 | 中 | 分阶段训练，控制在 ¥5,000-8,000 预算内 |
