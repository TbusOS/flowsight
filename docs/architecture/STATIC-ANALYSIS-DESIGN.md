...
│    - 直接调用：foo() 调用 bar()                                        │
│    - 确定的赋值：fp = handler; fp()                                    │
│    - 已知框架：usb_register() 的 probe 时机                            │
│                                                                          │
│  类型2：可能 (Possible)                                                 │
│  ═════════════════════                                                  │
│  • 显示方式：虚线箭头 - - - - → + 列出所有可能目标                     │
│  • 准确度：100%（列出的可能性是完整的，不遗漏）                        │
│  • 来源：                                                               │
│    - 条件赋值：if(cond) fp=a; else fp=b; → 可能是 a 或 b              │
│    - 数组索引：handlers[i]() → 可能是数组中任意元素                   │
│                                                                          │
│  类型3：未知 (Unknown)                                                  │
│  ═════════════════════                                                  │
│  • 显示方式：❓ 标记 + 提示用户补充信息                                │
...
|------|----------|--------|
| 直接调用 `foo()` | 语法分析确定 | **100%** |
| 确定赋值 `fp = func` | 追踪赋值 | **100%** |
| 条件赋值 `if(c) fp=a else fp=b` | **列出所有可能**：{a, b} | **100%** |
| 循环中赋值 | **列出所有可能** | **100%** |
| 已知框架 | 使用知识库 | **100%** |
| 外部库（无知识库） | **标注未知**，提示补充 | **100%** |
| 无法分析 | **标注未知**，询问用户 | **100%** |

**关键：不是"猜对多少"，而是"展示的都是对的"！**

### 7.3 与"猜测式分析"的区别

```
┌─────────────────────────────────────────────────────────────────────────┐
│                    错误做法 vs FlowSight 做法                            │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  错误做法（猜测式）：                                                   │
│  ───────────────────                                                    │
...
│  FlowSight 做法（严谨式）：                                             │
│  ─────────────────────────                                              │
│  • "fp 可能指向 {func_a, func_b, func_c}"（列出所有可能）              │
│  • 或者 "fp 指向的函数未知，需要补充信息"                              │
│  • 用户看到的每条信息都是 100% 可信的                                  │
│  • ✅ 正确！宁可说不知道，也不给错误信息！                             │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
...
│  对比人工阅读：                                                         │
│  ───────────────                                                        │
│  • 人工阅读：可能遗漏隐蔽的 callback 关系                              │
│  • FlowSight：不会遗漏，覆盖全部代码，不确定的明确标出                 │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```
