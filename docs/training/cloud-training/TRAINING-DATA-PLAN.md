...
> ⚠️ **注意**：AI 训练是项目 **Phase 4** 的工作，当前优先完成 IDE 核心功能。
> 本文档是训练计划的完整规划，实际执行在 IDE 核心功能完成后进行。

## 目录

1. [总体目标](#1-总体目标)
2. [训练策略与迭代计划](#2-训练策略与迭代计划)
3. [覆盖范围](#3-覆盖范围)
4. [数据规模估算](#4-数据规模估算)
5. [分阶段实施计划](#5-分阶段实施计划)
6. [数据质量标准](#6-数据质量标准)
...
#### Q1: 是否可以先训练 Linux 内核，发布第一版，后续迭代训练？
...
│  v1.0 - Linux 内核专用版                                                │
│  ════════════════════════                                               │
│  • 训练数据：70,000 样本（Linux 内核）                                  │
│  • 覆盖范围：完整 Linux 内核子系统                                      │
│  • 目标用户：Linux 内核开发者、驱动开发者                               │
│  • 发布形式：内置专用模型                                               │
│                                                                          │
│                           ↓                                             │
│                                                                          │
...
│                           ↓                                             │
│                                                                          │
│  v2.0 - 扩展版本（Android/RTOS/通用 C）                                │
│  ════════════════════════════════════                                   │
│  • 在 v1.x 基础上继续训练                                               │
│  • 添加 Android 框架知识（Binder/HAL/JNI）                             │
│  • 添加 RTOS 知识（FreeRTOS/Zephyr）                                   │
│  • 添加通用 C 项目模式                                                  │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```
...
**增量训练的技术实现**：
- 使用 LoRA/QLoRA 在原有模型上继续训练
- 不需要重新训练整个模型
- 训练成本低（每次增量训练 ¥100-300）

#### Q2: 安卓系统是否也要训练？

**答案：v1.0 先不训练 Android，v2.0 再加入。**

```
v1.0 专注 Linux 内核的原因：
├── 1. Linux 内核是 Android 的基础
├── 2. 学会 Linux 内核后，理解 Android 更容易
├── 3. 缩小 v1.0 的范围，更快发布
└── 4. 验证技术方案后再扩展
...
**答案：建议预算 ¥3000-5000，可以训练得非常好。**

详见 [7. 成本预算](#7-成本预算)

### 2.2 版本规划

| 版本 | 训练内容 | 样本量 | 预计成本 | 时间线 |
|------|----------|--------|----------|--------|
| **v1.0** | Linux 内核完整 | 70,000 | ¥2000-3000 | IDE 完成后 |
| **v1.1** | + 用户反馈修正 | +5,000 | ¥300-500 | v1.0 后 3 个月 |
| **v1.2** | + 新内核版本 | +5,000 | ¥300-500 | v1.0 后 6 个月 |
| **v2.0** | + Android 框架 | +30,000 | ¥1500-2000 | v1.0 后 1 年 |

### 2.3 模型演进路线

```
DeepSeek-Coder-6.7B (基础模型)
        ↓
    全参数微调 (v1.0 训练)
        ↓
FlowSight-Linux-6.7B (v1.0 发布模型)
        ↓
    知识蒸馏
        ↓
FlowSight-Linux-1.3B (本地部署模型，~0.8GB)
        ↓
    增量训练 (v1.x)
        ↓
FlowSight-Linux-1.3B-v1.x
        ↓
    扩展训练 (v2.0)
        ↓
FlowSight-Universal-1.3B (支持 Linux + Android + 通用 C)
```
...
### 6.1 Phase 1：核心驱动框架 + 异步机制（10,000 样本）

#### 3.1.1 总线驱动框架（3,000 样本）
...
#### 3.4.1 GPIO/Pinctrl/PWM（2,000 样本）
...
### 7.1 v1.0 Linux 内核完整训练

#### 云 GPU 租用成本

| 项目 | 配置 | 时间 | 单价 | 总价 |
|------|------|------|------|------|
| **数据准备** | A100 40GB | 5 小时 | ¥15/小时 | ¥75 |
| **全参数微调** | A100 80GB | 30 小时 | ¥30/小时 | ¥900 |
| **知识蒸馏** | A100 80GB | 10 小时 | ¥30/小时 | ¥300 |
| **评估测试** | A100 40GB | 5 小时 | ¥15/小时 | ¥75 |
| **冗余时间** | - | 10 小时 | - | ¥300 |
| **小计** | | | | **¥1,650** |
...
| 项目 | 说明 | 成本 |
|------|------|------|
| **LLM API 辅助** | GPT-4/Claude 生成初始数据 | ¥500-800 |
| **人工审核** | 抽查 10% 样本 | ¥0 (自己做) |
| **小计** | | **¥500-800** |
...
| 版本 | 训练类型 | 预计成本 |
|------|----------|----------|
| v1.1 | LoRA 增量训练 | ¥300-500 |
| v1.2 | LoRA 增量训练 | ¥300-500 |
| v2.0 | 扩展训练 | ¥1,500-2,000 |
...
对比方案（每年成本）：
• 使用 GPT-4 API：¥5,000-10,000/年（按使用量计费）
• 使用 Claude API：¥3,000-8,000/年
• 自己训练专用模型：一次性 ¥3,000 + 年维护 ¥1,000
