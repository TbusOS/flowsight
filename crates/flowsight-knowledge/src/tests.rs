...
fn test_all_yaml_files_parse() {
    ...
    let known_issues = ["spi.yaml", "kthread.yaml", "core/irq.yaml", "imx.yaml"];
    ...
    println!(
        "Validated {} YAML files ({} passed, {} known issues)",
        file_count,
        pass_count,
        warnings.len()
    );
...
fn test_builtin_knowledge_base_loads() {
    ...
    assert!(
        kb.async_patterns.contains_key("work_struct"),
        "Should have work_struct pattern"
    );

    // Should have timer_list
    assert!(
        kb.async_patterns.contains_key("timer_list"),
        "Should have timer_list pattern"
    );
    ...
    println!(
        "Builtin KB: {} frameworks, {} async patterns, {} APIs",
        kb.frameworks.len(),
...
fn test_usb_driver_framework_complete() {
    ...
    let kb = KnowledgeBase::builtin();
    let usb = kb
        .get_framework("usb_driver")
        .expect("usb_driver should exist");
    ...
    for cb_name in &required_callbacks {
        ...
        assert!(
            last_node.is_user_entry,
            "{} call chain should end with user entry",
            cb_name
        );
    }
}
...
fn test_work_struct_pattern_matching() {
    ...
    let trigger_patterns = [r"queue_work\s*\(", r"schedule_work\s*\("];
    ...
    assert!(
        bind_re2.is_match(code_samples[1]),
        "Should match INIT_DELAYED_WORK"
    );
    ...
    assert!(
        trigger_re2.is_match(code_samples[2]),
        "Should match schedule_work"
    );
    assert!(
        trigger_re1.is_match(code_samples[3]),
        "Should match queue_work"
    );
...
fn test_timer_pattern_matching() {
    ...
    assert!(
        bind_re.is_match(code_samples[0]),
        "Should match timer_setup"
    );
    ...
    assert!(
        trigger_re1.is_match(code_samples[1]),
        "Should match mod_timer"
    );
    assert!(
        trigger_re2.is_match(code_samples[2]),
        "Should match add_timer"
    );
}
...
"#;
    ...
    assert!(
        result.is_some(),
        "Should identify my_usb_probe as a callback"
    );
...
fn test_execution_context_correctness() {
    ...
    if let Some(pattern) = kb.get_async_pattern("timer_list") {
        assert_eq!(
            pattern.context,
            ExecutionContext::SoftIrq,
            "Timer callback should run in SoftIrq context"
        );
        assert!(
            !pattern.context.can_sleep(),
            "Timer callback should NOT be able to sleep"
        );
    }
    ...
    if let Some(pattern) = kb.get_async_pattern("work_struct") {
        ...
        assert!(
            pattern.context.can_sleep(),
            "WorkQueue callback should be able to sleep"
        );
    }
}
...
fn test_call_chain_integrity() {
    ...
    if let Some(chain) = kb.get_callback_call_chain("usb_driver", "probe") {
        ...
        assert!(
            chain.nodes.len() >= 5,
            "USB probe chain should have at least 5 nodes"
        );

        // Should start with hub connect
        assert!(
            chain.nodes[0].function.contains("hub") || chain.nodes[0].function.contains("port"),
            "USB probe chain should start from hub"
        );

        // Should end with user entry
        assert!(
            chain.nodes.last().unwrap().is_user_entry,
        ...
        for (i, node) in chain.nodes.iter().enumerate() {
            if !node.is_user_entry {
                assert!(
                    node.file.is_some() || i == chain.nodes.len() - 1,
