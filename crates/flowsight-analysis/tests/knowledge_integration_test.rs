...
use flowsight_parser::{Parser, treesitter::TreeSitterParser};
...
fn test_knowledge_base_loads() {
    ...
    assert!(kb.get_framework("usb_driver").is_some(), "Should have usb_driver framework");
    
    // Should have file_operations framework
    assert!(kb.get_framework("file_operations").is_some(), "Should have file_operations framework");
    
    // Should have async patterns
    assert!(kb.get_async_pattern("work_struct").is_some(), "Should have work_struct async pattern");
    assert!(kb.get_async_pattern("timer_list").is_some(), "Should have timer_list async pattern");
}
...
fn test_usb_probe_call_chain() {
    ...
    assert!(callback.call_chain.is_some(), "Probe should have call chain");
    
    let chain = callback.call_chain.as_ref().unwrap();
    assert!(!chain.nodes.is_empty(), "Call chain should have nodes");
    assert!(chain.trigger_source.contains("USB"), "Trigger should mention USB");
}
...
"#;
    ...
    assert!(result.entry_points.contains(&"my_probe".to_string()), 
        "Should detect my_probe as entry point");
    
    // Should detect my_disconnect as an entry point
    assert!(result.entry_points.contains(&"my_disconnect".to_string()),
    ...
    assert!(my_probe.unwrap().is_callback, "my_probe should be marked as callback");
}
...
"#;
    ...
    assert!(!result.async_bindings.is_empty(), "Should detect async bindings");
    
    let work_binding = result.async_bindings.iter()
        .find(|b| b.handler == "my_work_handler");
    assert!(work_binding.is_some(), "Should detect my_work_handler binding");
    
    let binding = work_binding.unwrap();
    assert!(matches!(binding.mechanism, flowsight_core::AsyncMechanism::WorkQueue { .. }),
...
"#;
    ...
    assert!(timer_binding.is_some(), "Should detect my_timer_handler binding");
    
    let binding = timer_binding.unwrap();
    assert!(matches!(binding.mechanism, flowsight_core::AsyncMechanism::Timer { .. }),
        "Should be Timer mechanism");
    
    // Timer handlers run in softirq context
    assert_eq!(binding.context, flowsight_core::ExecutionContext::SoftIrq,
...
fn test_yaml_patterns_loaded() {
    ...
    let has_yaml_frameworks = kb.frameworks.keys()
        .any(|k| k.contains("usb_") || k.contains("platform_") || k.contains("i2c_"));
    
    // At minimum, built-in frameworks should exist
    assert!(kb.frameworks.contains_key("usb_driver"), "Should have usb_driver");
    assert!(kb.frameworks.contains_key("file_operations"), "Should have file_operations");
}
...
"#;
    ...
    assert!(probe_tree.is_some() || result.flow_trees.iter().any(|t| 
        t.name.contains("USB") || t.children.iter().any(|c| c.name == "my_probe")
...
"#;
    ...
    assert!(my_open.unwrap().is_callback, "my_open should be marked as callback");
    
    // Should have callback context
    let callback_ctx = my_open.unwrap().callback_context.as_ref();
    assert!(callback_ctx.is_some(), "my_open should have callback context");
}
