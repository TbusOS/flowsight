...
use flowsight_analysis::flow_builder::{BuildOptions, FlowBuilder, FunctionInfo};
...
"#;
    ...
    let probe = result
        .functions
        .get("amd8111_gpio_probe")
        .expect("应该找到 probe 函数");

    // 应该包含这些调用
    assert!(
        probe.calls.contains(&"devm_kzalloc".to_string()),
        "应该提取 devm_kzalloc 调用，实际: {:?}",
        probe.calls
    );
    assert!(
        probe.calls.contains(&"gpiochip_add_data".to_string()),
        "应该提取 gpiochip_add_data 调用，实际: {:?}",
        probe.calls
    );
    assert!(
        probe.calls.contains(&"pci_set_drvdata".to_string()),
        "应该提取 pci_set_drvdata 调用，实际: {:?}",
        probe.calls
    );
    ...
    let gpio_set = result
        .functions
        .get("amd_gpio_set")
        .expect("应该找到 gpio_set 函数");
    assert!(
        gpio_set.calls.contains(&"spin_lock_irqsave".to_string()),
        "应该提取 spin_lock_irqsave 调用，实际: {:?}",
        gpio_set.calls
    );
