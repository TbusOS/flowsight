...
impl FeedbackCollector {
    ...
    pub fn get_all(&self) -> Result<Vec<UserFeedback>, FeedbackError> {
        let mut stmt = self.conn.prepare("SELECT * FROM feedback ORDER BY timestamp DESC")?;
        let rows = stmt.query_map((), |row| {
            let content_json: String = row.get(6)?;
            let content: FeedbackContent = serde_json::from_str(&content_json)
                .unwrap_or(FeedbackContent::Correctness {
                    is_correct: true,
                    correct_answer: None,
            ...
            Ok(UserFeedback {
                id: row.get(0)?,
                feedback_type: serde_json::from_str(&row.get::<_, String>(1)?).unwrap_or(FeedbackType::Correctness),
    ...
    pub fn count(&self) -> Result<usize, FeedbackError> {
        let count: i64 = self.conn.query_row("SELECT COUNT(*) FROM feedback", (), |row| row.get(0))?;
        Ok(count as usize)
    }
    ...
    pub fn by_function(&self, function: &str) -> Result<Vec<UserFeedback>, FeedbackError> {
        let mut stmt = self.conn.prepare("SELECT * FROM feedback WHERE function = ? ORDER BY timestamp DESC")?;
    ...
    pub fn delete_old(&self, keep: usize) -> Result<usize, FeedbackError> {
        ...
        let ids_to_keep: Result<Vec<String>, rusqlite::Error> = stmt
...
impl UserFeedback {
    ...
    pub fn to_training_sample(self) -> Option<TrainingSample> {
        match self.content {
            FeedbackContent::Correctness { is_correct, correct_answer, error_description } => {
                Some(TrainingSample {
                    instruction: format!("分析以下代码中函数的执行是否正确：{}", self.function),
                    input: self.code_snippet,
                    output: correct_answer.unwrap_or_else(|| {
                        if is_correct { "正确".into() } else { error_description.unwrap_or("错误".into()) }
                    }),
                    feedback_type: "correctness".into(),
                })
            }
            FeedbackContent::KnowledgeSupplement { pattern, explanation, trigger_condition, .. } => {
                Some(TrainingSample {
            ...
            FeedbackContent::PatternDiscovery { snippet, pattern_type, semantics, .. } => {
                Some(TrainingSample {
            ...
            FeedbackContent::PathSelection { path_taken, reason, .. } => {
                Some(TrainingSample {
            ...
            FeedbackContent::TranslationCorrection { original, corrected, reason } => {
                Some(TrainingSample {
